...
class revive(commands.Cog):
    ...
    @commands.command(name='revive', description="Generate a topic and ping dead chat")
    @commands.has_permissions(manage_messages=True)
    @app_commands.default_permissions(manage_messages=True)
    async def revive(self, ctx, channel: discord.TextChannel = None):
        if channel is None:
        ...
        if channel.guild.id == 1081122313870778471:
            ...
            await channel.send(f"# <@&1224606858367471667> {random_starter}")
        elif channel.guild.id == 1307977167564574730:
                random_starter = random.choice(starters)
                await channel.send(f"# <@&1320870144821563513> {random_starter}")
        else:
            await ctx.send("This command is not compatible with this guild!", ephemeral=True)


    @app_commands.command(name= 'revive', description= "Generate a topic and ping dead chat ping")
    @app_commands.default_permissions(manage_messages=True)
    async def slash_revive(self, interaction: discord.Interaction, channel: discord.channel.TextChannel=None):
        """
        Parameters
        -----------
